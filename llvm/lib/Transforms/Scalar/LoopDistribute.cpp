...
public:
  ...
  bool processLoop() {
    ...
    if (!LAI->hasLoadStoreDependenceInvolvingLoopInvariantAddress() && LAI->canVectorizeMemory())
      return fail("MemOpsCanBeVectorized",
                  "memory operations are safe for vectorization");

    auto *Dependences = LAI->getDepChecker().getDependences();
    if (!LAI->hasLoadStoreDependenceInvolvingLoopInvariantAddress() && (!Dependences || Dependences->empty()))
